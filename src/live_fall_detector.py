...
# Ensure you are importing the updated window_vector_nan that handles speed
...
class LiveManDownDetector:
    def __init__(
        ...
        if isinstance(min_window_quality, str):
            self.min_window_quality = quality_levels.get(min_window_quality.lower(), 0.80)
        else:
        ...
        self.post_fall_duration = int(post_fall_duration)
    ...
    def update(self, frame_landmarks: Union[Pose33, PoseCell]) -> Dict[str, Any]:
        ...
        if frame_landmarks is None:
            if self.reset_on_invalid: self.reset()
        ...
        fall_ok, h_ok = self._ingest(pose33)
        ...
        # 3. Check Fall Quality
        fall_q = self._window_quality(self._fall_q_buf)
        if fall_q < self.min_window_quality and self._post_fall_timer == 0:
            self._fall_hits = 0
            self._horiz_hits = 0
            return {
                "ready": False,
